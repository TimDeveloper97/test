...
{
    public class GeneralCheckElectronic
    {
        private QLTKEntities db = new QLTKEntities();

        public string GeneralTempalteCheckElectronic(GeneralElectronicModel model)
        {
            ...
            string templatePath = HttpContext.Current.Server.MapPath("~/Template/KiemTraChatLuongSPMachDienTu.docm");
            using (WordDocument document = new WordDocument(templatePath))
            {
                ...
                string pathFileSave = HttpContext.Current.Server.MapPath("~/Template/" + Constants.FolderExport + "KiemTraChatLuongSanPhamMachDienTu_" + specifyName + ".docm");
                document.Save(pathFileSave);

                UserLogUtil.LogHistotyAdd(db, model.CreateBy, model.ProductCode, null, Constants.LOG_GeneralCheckElectronic);

                document.Close();
            }
            string pathreturn = "Template/" + Constants.FolderExport + "KiemTraChatLuongSanPhamMachDienTu_" + specifyName + ".docm";
