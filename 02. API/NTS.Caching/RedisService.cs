...
namespace NTS.Caching
{
    public class RedisService<T>
    {
        ...
        private RedisService()
        {
            ...
            this.Server = ConnectionFactory.Connection().GetServer(ConnectionFactory.Connection().GetEndPoints().First());
        }
        ...
        public static RedisService<T> GetInstance()
        {
            if (singletonObject == null)
            {
                singletonObject = new RedisService<T>();
        ...
        public List<T> GetContains(string key)
        {
            List<T> listObject = new List<T>();
            try
            {
                var objectRedisKey = Server.Keys(pattern: key);
                foreach (var redisKey in objectRedisKey)
                {
                    listObject.Add(this.Get<T>(redisKey.ToString()));
                }
                return listObject;
            }
            catch (TimeoutException ex)
        ...
        public bool Add(string key, string valueJson, TimeSpan expiresIn)
        {
            return this.Database.StringSet(key, valueJson, new TimeSpan?(expiresIn), When.Always, CommandFlags.None);
        }
