...
{
    public class GeneralPrinciplesCalculate
    {
        private QLTKEntities db = new QLTKEntities();

        public string GeneralTemplatePrinciplesCalculate(GeneralElectronicModel model)
        {
            ...
            string templatePath = HttpContext.Current.Server.MapPath("~/Template/BieuMauKiemTraNguyenLy_2.docm");
            using (WordDocument document = new WordDocument(templatePath))
            {
                //WTable table = document.GetTableByFindText("<data>");
                ...
                string pathFileSave = HttpContext.Current.Server.MapPath("~/Template/" + Constants.FolderExport + "BieuMauKiemTraNguyenLy_2_" + specifyName + ".docm");

                document.Save(pathFileSave);

                UserLogUtil.LogHistotyAdd(db, model.CreateBy, model.ProductCode, null, Constants.LOG_GeneralPrinciplesCalculate);

                document.Close();
            }
            string pathreturn = "Template/" + Constants.FolderExport + "BieuMauKiemTraNguyenLy_2_" + specifyName + ".docm";
