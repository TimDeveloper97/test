...
{
    public class GeneralDesignOptions
    {
        private QLTKEntities db = new QLTKEntities();

        public string GeneralTempalteDesignOptions(GeneralElectronicModel model)
        {
            ...
            const string sp = "Tên sản phẩm";
            const string msp = "Mã sản phẩm";
            string templatePath = HttpContext.Current.Server.MapPath("~/Template/PATK_SoDoKhoi.docm");
            ...
            using (WordDocument document = new WordDocument(templatePath))
            {
                ...
                document.NTSReplaceAll("Tên sản phẩm", sp);
                document.NTSReplaceAll("Mã sản phẩm", msp);
                ...
                string pathFileSave = HttpContext.Current.Server.MapPath("~/Template/" + Constants.FolderExport + "PATK_SoDoKhoi_" + specifyName + ".docm");
                document.Save(pathFileSave);


                UserLogUtil.LogHistotyAdd(db, model.CreateBy, model.ProductCode, null, Constants.LOG_GeneralDesignOptions);

                document.Close();
            }

            string pathreturn = "Template/" + Constants.FolderExport + "PATK_SoDoKhoi_" + specifyName + ".docm";
