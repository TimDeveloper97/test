namespace CoreFtp
{
    ...
    public class FtpClient : IFtpClient
    {
        ...
        /// <summary>
        /// Creates a directory on the FTP Server not Recursively
        /// </summary>
        /// <param name="directory"></param>
        /// <returns></returns>
        public async Task CreateDirectoryNotRecursivelyAsync(string directory)
        {
            if (directory.IsNullOrWhiteSpace() || directory.Equals("."))
                throw new ArgumentOutOfRangeException(nameof(directory), "Directory supplied was not valid");
        ...
        /// <summary>
        /// Provides a stream which can be written to
        /// Không kiểm tra thư mục
        /// </summary>
        /// <param name="fileName"></param>
        /// <returns></returns>
        public async Task<Stream> OpenFileWriteStreamNoCheckDirectoryAsync(string fileName)
        {
            string filePath = WorkingDirectory.CombineAsUriWith(fileName);
            Logger?.LogDebug($"[FtpClient] Opening file read stream for {filePath}");
            var segments = filePath.Split('/')
