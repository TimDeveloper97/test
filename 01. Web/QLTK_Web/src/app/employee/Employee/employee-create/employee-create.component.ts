...
import { AppSetting, MessageService, Constants, FileProcess, DateUtils, ComboboxService } from 'src/app/shared';
import { EmployeeCreateServiceService } from '../../service/employee-create-service.service';
...
export class EmployeeCreateComponent implements OnInit {
  constructor(
    ...
    private employeeServiceCreate: EmployeeCreateServiceService,
  ...
  deleteJobTranfer(index: any) {
    this.messageService.showConfirm("Bạn có chắc chắn muốn xóa lịch sử điều chuyển này không?").then(
      data => {
  ...
  save(isContinue: boolean) {
    ...
    forkJoin([filesDecisionTranfer, filesDecisionAppoint, filesContract]).subscribe(results => {
      ...
      if (this.fileProcess.FileDataBase == null) {
        this.createEmployee(isContinue);
      }
      else {
        let modelFile = {
          FolderName: 'Employee/'
        };
        this.employeeServiceCreate.uploadImage(this.fileProcess.FileDataBase, modelFile).subscribe(
          data => {
  ...
  createEmployee(isContinue: boolean) {
    ...
    this.employeeServiceCreate.CreateEmployee(this.model).subscribe(
      data => {
        if (validEmail) {
          if (isContinue) {
            this.isAction = true;
