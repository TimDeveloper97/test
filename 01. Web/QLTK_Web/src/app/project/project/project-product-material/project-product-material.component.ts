...
@Component({
  selector: 'app-project-product-material',
  templateUrl: './project-product-material.component.html',
  styleUrls: ['./project-product-material.component.scss']
})
export class ProjectProductMaterialComponent implements OnInit {
  ...
  searchModuleMaterial() {
    ...
    this.projectProductService.searchModuleMaterial(this.modelModuleMaterial).subscribe((data: any) => {
      if (data.ListResult) {
        ...
        //   if(element.ModuleName != null && element.ModuleName !=""){
          ...
          if(element.Index !=null){
            if(element.Index.indexOf(".") == -1){
              var resultModel ={
                ...
                parentTotalPrice : element.Quantity*element.Pricing,
                results :[],
              }
        ...
        this.listData.forEach(element =>{
          resultModels.forEach(e =>{
            if(element.Index !=null && e.parentIndex !=null){
        ...
        resultModels.forEach(element =>{
          ...
          element.results.forEach(e =>{
            total = total + (e.Quantity*e.Pricing);
          });
  ...
  exportExcel() {
    this.serviceModuleMaterial.exportExcelMaterialBOMDraft(this.modelModuleMaterial).subscribe(d => {
      var link = document.createElement('a');
      link.setAttribute("type", "hidden");
